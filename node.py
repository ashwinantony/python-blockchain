...
def broadcast_transaction():
    ...
    if success:
        response = {
            'message': 'Successfuly added new transaction.!',
            'transaction': {
        ...
        response = {
            'message': 'Creating new transaction failed.!'
        }
        return jsonify(response), 500
...
def broadcast_block():
    ...
    if block['index'] == blockchain.chain[-1].index + 1:
        ...
        response = {
            'message': 'Blockchain differ from Local Blockchain. Block not added..!'}
...
if __name__ == '__main__':
    ...
    # Execute node.py file with -p port_number or --port port_number as arguments
    parser.add_argument('-p', '--port', type=int, default=5000)
    args = parser.parse_args()
    # Access port number and assign it to port variable
    port = args.port

    # Object creation for each port/Node
    wallet = Wallet(port)
    blockchain = Blockchain(wallet.public_key, port)
