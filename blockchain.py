...
class Blockchain:
    ...
    def __init__(self, public_key, node_id):
        ...
        # Initializing unhandled transactions
        self.__open_transactions = []
        self.pubilc_key = public_key
    ...
    @property
    def chain(self):
        # returns a copy of chain attribute
        return self.__chain[:]

    # The setter for the chain property
    @chain.setter
    def chain(self, val):
        # update chain attribute value with passed 'val'
        self.__chain = val
    ...
    def load_data(self):
        """Initialize blockchain and Open transactions data from a blockchain_json.txt file"""
        try:
            # Method 1: PICKLE file operations -> No need to worry about OrderedDict conversion
            # with open('blockchain_pickle.txt', mode='rb') as f:
            #     file_content = pickle.loads(f.read())
            #     blockchain = file_content['chain']
            #     open_transactions = file_content['open_tx']

            # Method 2: JSON file operations -> Need to take care of manual OrderedDict conversion
            with open('blockchain-{}_json.txt'.format(self.node_id), mode='r') as f:
                file_content = f.readlines()
                blockchain = json.loads(file_content[0][:-1])
                # Resolving Data-mismatch due to the use of OrderedDict()
                updated_blockchain = []
            ...
            print('File blockchain_json.txt not Found!')
        finally:
            print('Clean-up!')
    ...
    def save_data(self):
        ...
        try:
            # Method 1: PICKLE file operations -> No need to worry about OrderedDict conversion
            # with open('blockchain_pickle.txt', mode='wb') as f:
            #     save_data = {
            #         'chain': blockchain,
            ...
            # Method 2: JSON file operations -> Need to take care of manual OrderedDict conversion
            with open('blockchain-{}_json.txt'.format(self.node_id), mode='w') as f:
                saveable_chain = [
    ...
    def get_balance(self, sender=None):
        ...
        if sender is None:
            if self.pubilc_key is None:
                return None
            participant = self.pubilc_key
        else:
            participant = sender
        ...
        # Adding open_transaction amount:
        # Fetch a list of all sent coin amounts for the given person (empty
        # lists are returned if the person was NOT the sender)
    ...
    def add_transaction(self,
        ...
        #     'amount': amount
        # }
        # if self.pubilc_key == None:
        #     return False

        # OrderedDict() -> used to overcome the unordered property
        # of normal Dict while hashing each time
    ...
    def mine_block(self):
        ...
        # Generating temp Hash value => Hash the last block
    ...
    def add_block(self, block):
        ...
        # Validate the proof of work of the block and store the result
        ...
        for node in self.__peer_nodes:
            ...
            try:
                ...
                if (node_chain_length > local_chain_length and
                    ...
                    # if we find out that we are replacing our local block chain
                    # then we will also assume that our transactions are incorrect
                    # because we might have tried to spend more money than we have.
                    # So no need to keep these around.
                    replace = True
            except requests.exceptions.ConnectionError:
                continue
        self.resolve_conflicts = False
